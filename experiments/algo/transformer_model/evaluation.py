...
def print_stat_v2(data_frame, real_column, prediction_column):
    data_frame = data_frame.sort_values(real_column)
    ...
    return rmse_value, mae, pearson, spearman
...
def print_binary_stat(df, real_column, pred_column):
    ...
    recall_result = recall_score(real_values, predictions, average='weighted')
    precision_result = precision_score(real_values, predictions, average='weighted')
    f1_score_result = f1_score(real_values, predictions, average='weighted')

    
    return recall_result, precision_result, f1_score_result


def return_binary_stat(df, real_column, pred_column):
    predictions = df[pred_column].tolist()
    real_values = df[real_column].tolist()
