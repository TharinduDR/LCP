...
for i in range(1):
    model_args = LCPArgs()
    model_args.best_model_dir = "english_transfer_outputs/best_model"
...
portuguese_train = pd.read_csv("data/v0.02_MultiLex_train.tsv", sep="\t")
portuguese_dev = pd.read_csv("data/v0.02_MultiLex_dev.tsv", sep="\t")
portuguese_test = pd.read_csv("data/v0.02_MultiLex_test.tsv", sep="\t")

portuguese_train = portuguese_train[["genre", "pt_sentence", "pt_word", "avg_complexity"]]
portuguese_dev = portuguese_dev[["genre", "pt_sentence", "pt_word", "avg_complexity"]]
portuguese_test = portuguese_test[["genre", "pt_sentence", "pt_word", "avg_complexity"]]

portuguese_train["text_a"] = portuguese_train["genre"] + ' ' + portuguese_train["pt_word"]
portuguese_dev["text_a"] = portuguese_dev["genre"] + ' ' + portuguese_dev["pt_word"]
portuguese_test["text_a"] = portuguese_test["genre"] + ' ' + portuguese_test["pt_word"]

portuguese_train = portuguese_train.rename(columns={'pt_sentence': 'text_b', 'avg_complexity': 'labels'}).dropna()
portuguese_dev = portuguese_dev.rename(columns={'pt_sentence': 'text_b', 'avg_complexity': 'labels'}).dropna()
portuguese_test = portuguese_test.rename(columns={'pt_sentence': 'text_b', 'avg_complexity': 'labels'}).dropna()

portuguese_train = portuguese_train[["text_a", "text_b", "labels"]]
portuguese_dev = portuguese_dev[["text_a", "text_b", "labels"]]
portuguese_test = portuguese_test[["text_a", "text_b", "labels"]]

portuguese_test_sentence_pairs = list(
    map(list, zip(portuguese_test['text_a'].to_list(), portuguese_test['text_b'].to_list())))
...
for i in range(5):
    ...
    model = LCPModel(model_args.model_type, model_args.model_name, num_labels=1, use_cuda=torch.cuda.is_available(),
                     args=model_args)
    zero_portuguese_predictions, zero_portuguese_raw_outputs = model.predict(portuguese_test_sentence_pairs)

    portuguese_test['zero_predictions'] = zero_portuguese_predictions
    print_stat(portuguese_test, 'labels', 'zero_predictions')
    ...
    for genre in genres:
        ...
        print_stat(filtered_predictions, 'labels', 'zero_predictions')
        print("=================")

    model.train_model(portuguese_train, eval_df=portuguese_dev, pearson_corr=pearson_corr, spearman_corr=spearman_corr,
                      mae=mean_absolute_error)
    model = LCPModel(model_args.model_type, model_args.best_model_dir, num_labels=1,
                     use_cuda=torch.cuda.is_available(), args=model_args)
...
print_stat(portuguese_test, 'labels', 'predictions')
portuguese_test.to_csv("test_predictions.csv", sep="\t", index=False)
